import 'package:test/test.dart';
import 'package:ym_api_client/ym_api_client.dart';

// tests for AdvertPublicFavoriteEntity
void main() {
  final AdvertPublicFavoriteEntity? instance = /* AdvertPublicFavoriteEntity(...) */ null;
  // TODO add properties to the entity

  group(AdvertPublicFavoriteEntity, () {
    // int id
    test('to test the property `id`', () async {
      // TODO
    });

    // bool purchase
    test('to test the property `purchase`', () async {
      // TODO
    });

    // bool published
    test('to test the property `published`', () async {
      // TODO
    });

    // num moderate
    test('to test the property `moderate`', () async {
      // TODO
    });

    // num price
    test('to test the property `price`', () async {
      // TODO
    });

    // int quantity
    test('to test the property `quantity`', () async {
      // TODO
    });

    // bool contractPrice
    test('to test the property `contractPrice`', () async {
      // TODO
    });

    // int productId
    test('to test the property `productId`', () async {
      // TODO
    });

    // int ownerId
    test('to test the property `ownerId`', () async {
      // TODO
    });

    // String countryId
    test('to test the property `countryId`', () async {
      // TODO
    });

    // int regionId
    test('to test the property `regionId`', () async {
      // TODO
    });

    // int cleaningId
    test('to test the property `cleaningId`', () async {
      // TODO
    });

    // int packingId
    test('to test the property `packingId`', () async {
      // TODO
    });

    // int qualityId
    test('to test the property `qualityId`', () async {
      // TODO
    });

    // int recyclingId
    test('to test the property `recyclingId`', () async {
      // TODO
    });

    // DateTime publicationDate
    test('to test the property `publicationDate`', () async {
      // TODO
    });

    // String info
    test('to test the property `info`', () async {
      // TODO
    });

    // String infoRu
    test('to test the property `infoRu`', () async {
      // TODO
    });

    // String address
    test('to test the property `address`', () async {
      // TODO
    });

    // num lat
    test('to test the property `lat`', () async {
      // TODO
    });

    // num lon
    test('to test the property `lon`', () async {
      // TODO
    });

    // DateTime updatedAt
    test('to test the property `updatedAt`', () async {
      // TODO
    });

    // ProductEntity product
    test('to test the property `product`', () async {
      // TODO
    });

    // UserPublicEntity user
    test('to test the property `user`', () async {
      // TODO
    });

    // CleaningEntity cleaning
    test('to test the property `cleaning`', () async {
      // TODO
    });

    // PackingEntity packing
    test('to test the property `packing`', () async {
      // TODO
    });

    // QualityEntity quality
    test('to test the property `quality`', () async {
      // TODO
    });

    // RecyclingEntity recycling
    test('to test the property `recycling`', () async {
      // TODO
    });

    // CountryEntity country
    test('to test the property `country`', () async {
      // TODO
    });

    // RegionEntity region
    test('to test the property `region`', () async {
      // TODO
    });

    // AdvertBuyPublicEntity advertBuy
    test('to test the property `advertBuy`', () async {
      // TODO
    });

    // bool isFavorite
    test('to test the property `isFavorite`', () async {
      // TODO
    });

  });
}
